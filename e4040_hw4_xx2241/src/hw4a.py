...
class RNNSLU(object):
    ...
    def __init__(self, nh, nc, ne, de, cs, normal=True):
        ...
        def recurrence(x_t, h_tm1):
            ...
            theta_t = T.sqrt(T.mean(T.sqr(a_t-T.mean(a_t))))
            h_t = T.nnet.sigmoid((self.g/theta_t)*(a_t-T.mean(a_t))+self.bh)
...
class RNNSLU2(object):
    ...
    def __init__(self, nh, nh2, nc, ne, de, cs, normal=True):
        ...
        self.h0 = theano.shared(name='h0',
                                value=numpy.zeros(nh,
                                dtype=theano.config.floatX))
        self.h02 = theano.shared(name='h0',
                                value=numpy.zeros(nh2,
                                dtype=theano.config.floatX))

        # bundle
        self.params = [self.emb, self.wx,self.wx2, self.wh,self.wh2, self.w2,
                       self.bh, self.bh2,self.b, self.h0, self.h02]
        ...
        def recurrence(x_t, h_tm1,h_tm2):
            ...
            h_t2 = T.nnet.sigmoid(T.dot(h_t, self.wx2) + T.dot(h_tm2, self.wh2) + self.bh2)
            s_t = T.nnet.softmax(T.dot(h_t2, self.w2) + self.b)
            return [h_t, h_t2, s_t]

        [h0, h, s], _ = theano.scan(fn=recurrence,
                                sequences=x,
...
def test_rnnslu(**kwargs):
    ...
    return rnn.emb.eval(), idx2label, idx2word
