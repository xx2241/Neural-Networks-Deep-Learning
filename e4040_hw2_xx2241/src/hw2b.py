...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, activation, layernums):
        """Initialize the parameters for the multilayer perceptron
        ...
        for i in range(layernums):
            ...
            self.hiddenLayerList.append(self.hiddenLayer);
        ...
        # of the hidden layer
        self.logRegressionLayer = LogisticRegression(
            input=self.hiddenLayerList[layernums-1].output,
            n_in=n_hidden,
            n_out=n_out
        )
        # L1 norm ; one regularization option is to enforce L1 norm to
        # be small
        sumL1 = abs(self.hiddenLayerList[0].W).sum()
        for i in range(1, layernums):
            sumL1 = sumL1 + (self.hiddenLayerList[i].W).sum()
        ...
        # square of L2 norm ; one regularization option is to enforce
        # square of L2 norm to be small
        sumL2 = (self.hiddenLayerList[0].W**2).sum()
        for i in range(1, layernums):
            sumL2 = sumL2 + (self.hiddenLayerList[i].W**2).sum()
        ...
        # the parameters of the model are the parameters of the two layer it is
        # made out of
        sumparams = self.hiddenLayerList[0].params
        for i in range(1, layernums):
            sumparams = sumparams +  self.hiddenLayerList[i].params
        
        self.params = sumparams + self.logRegressionLayer.params
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hidden=500, verbose=False, activation= T.tanh, layernums=2):
    """
